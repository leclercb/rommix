...
Eleven attributes exist for `npm run test:app` and nothing else: `data-screen` on
...
`data-status` on one answer in the status dialog, and `data-action` on a
`FocusButton` that a scenario presses. They are there because
