...
Thirteen attributes exist for `npm run test:app` and nothing else:
...
thumbnail, `data-shelf` on a row of games on the home screen, and `data-action`
on a `FocusButton` that a scenario presses. They are there because
